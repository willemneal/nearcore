    ...
    pub fn process_chunk_one_part(
        ...
        for shard_id in 0..self.runtime_adapter.num_shards() {
            if self.cares_about_shard_this_or_next_epoch(
                ...
                if !verify_path(one_part.header.receipts_root, &one_part.receipts_proofs[i], &receipts_hashes[shard_id as usize]) {
                    assert!(false); // TODO XXX MOO remove
                    return Err(ErrorKind::Other("Invalid merkle proof for receipts".to_string()).into());
                }
                i += 1;
            }
        }
        if i != one_part.receipts_proofs.len() {
            assert!(false); // TODO XXX MOO remove
            return Err(ErrorKind::Other("Invalid merkle proof for receipts".to_string()).into());
        }
